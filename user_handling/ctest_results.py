...
async def redirect_res_auth(request: Request, ctest_id: str, code: str = Form(...), db: Session = Depends(get_db)):
    ...
    if otp_entry and otp_entry.teacher_code == code:
        request.session[f"res_auth_{ctest_id}"] = True
        return RedirectResponse(f"/results/{ctest_id}", status_code=302)
    else:
...
async def get_results(request: Request, ctest_id: str, db: Session = Depends(get_db)):
    ...
    Returns:
        ...
        TemplateResponse: expired.html if test/submission not found (410)

    Raises:
        HTTPException: 500 for server errors during rendering
    """
    try:
        if not request.session.get(f"res_auth_{ctest_id}"):
            return RedirectResponse(f"/results_authorize/{ctest_id}", status_code=302)
